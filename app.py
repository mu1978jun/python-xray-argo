...
UUID = os.environ.get('UUID', 'd9893ccb-3227-4b3a-ab22-26bfbed89a9b')  # UUID,如使用哪吒v1,在不同的平台部署需要修改,否则会覆盖
...
ARGO_DOMAIN = os.environ.get('ARGO_DOMAIN', 'streamlit.ccuu.de5.net')        # Argo固定隧道域名,留空即使用临时隧道
ARGO_AUTH = os.environ.get('ARGO_AUTH', 'eyJhIjoiOTQxMTg5MzAzZTAzN2ZlMmUxYjgwZWIyZjgwODM1ZGEiLCJ0IjoiMzAwNzc4OGMtYThiMy00NzYzLTgwYjItZWU0ZDk2NTdiYWQyIiwicyI6IllXVmlZMlZsT0RNdFptVm1PUzAwWlRSa0xXRTFPV010TjJNelptUmhPRGhrTURnMiJ9')            # Argo固定隧道密钥,留空即使用临时隧道
ARGO_PORT = int(os.environ.get('ARGO_PORT', '49999'))   # Argo端口,使用固定隧道token需在cloudflare后台设置端口和这里一致
